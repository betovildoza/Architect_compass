...
# Backlog 2026-06-14 — tickets nuevos + actualizaciones (S25, Fase B)

Origen: 3 verificaciones empíricas de campo en grafos reales durante S25 (no scanner aislado), disparadas por observaciones directas de Beto sobre los `graph.html` de ETCA y clases.etca. Reportes: `~/.claude/results/reviewer-compass-etca-css-sueltos-20260614.md`, `reviewer-compass-etca-censo-huerfanos-y-cruce-backlog-20260614.md`, `reviewer-compass-clasesetca-nextjs-huerfanos-20260614.md`, `architect-compass-web039-clase-analisis-20260614.md`. Cada hallazgo se cruzó contra el backlog existente (anti-duplicación) antes de ticketizar. El índice vive en [roadmap.md](roadmap.md); acá el detalle.

> **Lección metodológica de S25 (canónica):** para edges HTML→asset la verdad está en `connectivity.outbound` del atlas TRAS la resolución, NUNCA en el output del scanner aislado. El primer diagnóstico de Fase B declaró CSS-049 "ya implementado" probando el scanner suelto; la observación de Beto sobre ETCA lo refutó. Toda afirmación de "conecta" debe verificarse en grafo real.

## MARKUP-061 — Markup embebido en templates server-side ✅completada (S25)

> **Resultado S25 (2026-06-15):** implementado y validado por reviewer independiente en grafo real de ETCA. Los 6 nodos `.php` conectan (`_tienda.css`, `_producto.css`, `_tienda-categorias.css` vía `<link>`; `tienda-products.js`, `tienda-cart.js`, `producto.js` vía `<script src>`), edge_type `href`/`src` (EDG-023 intacto). `_shared.css` citado en comentarios sigue huérfano. No-regresión: 0 edges removidos, SEM-020 14 enqueue intactos, tiers idénticos. Filtrado transversal de comentarios funcionando en HTML/CSS/JS/TS/PHP (helper `compass/comment_filter.py` string-aware), Python exento (AST). Fase 3-fases con checkpoint humano en cada transición. Reportes: `implementer-compass-markup061-20260614.md`, `reviewer-compass-markup061-review-20260614.md`.
>
> **Nota técnica adjunta — markup-pass usa HtmlScanner regex, NO tree-sitter (NO es desvío aprobado, es información registrada con su disparador):** el diseño v1 §2.2 pedía "mejor tier disponible" (tree-sitter si está). El implementer lo cambió a SIEMPRE regex porque la grammar HTML tree-sitter sobre un `.php` **colapsa**: evidencia en `tienda.php` = **1639 nodos ERROR, 0 `element`, 0 items extraídos** (vs 21 del regex), por el `<?php ?>` embebido. Verificado independientemente por el reviewer (mismos números). El regex es robusto a HTML+PHP mixto. No es deuda: es la decisión correcta para templates server-side, documentada en la docstring de `get_markup_scanner`. **Implicancia para las extensiones sin testigo** (`.twig/.erb/.jsp/.ejs`): comparten la misma propiedad (markup mezclado con código de template rompe el AST) → el markup-pass regex es el camino correcto también para ellas cuando aparezca un testigo. Esta nota ES su evidencia disparadora si en el futuro se reabre el tema del tier del markup-scanner.
>
> **Extensiones cubiertas por clase pero NO verificadas (sin testigo en portfolio):** `.twig`, `.blade.php`, `.erb`, `.jsp`, `.ejs`. El código las maneja (lista `HTML_BEARING_EXTENSIONS` en `defaults.py`), pero NO se declaran ✅-verificadas hasta tener un proyecto testigo (regla canónica: no afirmar sin comprobar en grafo real). Candidatos a testigo en el portfolio: WP-plugin de ETCA (PHP con posible markup inline — a auditar).

### Especificación original (conservada)


- **Origen:** observación de Beto (CSS sueltos en `etca.com.ar/.map`) → censo empírico S25. ETCA expone el bug por ser híbrido: páginas en `.html` conectan, páginas en `.php` no.
- **Síntoma:** un recurso frontend referenciado con `<link rel="stylesheet" href="...">` o `<script src="...">` **dentro de un archivo `.php`** queda suelto. El edge nunca se emite. Confirmado: 6 nodos en ETCA — `_tienda.css`, `_producto.css`, `_tienda-categorias.css` (vía `<link>` en `tienda.php`/`producto.php`) + `tienda-products.js`, `tienda-cart.js`, `producto.js` (vía `<script src>` en los mismos `.php`). Prueba dura: `index.html:133` con `href="/css/_base.css"` resuelve (14 inbound); `producto.php:111` con href idéntico NO genera edge.
- **Causa raíz (estructural, confirmada en código):** el dispatch de scanner ata la extracción de markup a `language ∈ {html,htm}`. `compass/scanners/__init__.py:102` invoca `HtmlScanner`/`TreeSitterHtmlScanner` solo en ese caso; `compass/pipeline.py:40` mapea `.php → language="php"` → va al scanner PHP, que extrae `require`/`include`/`http_loaders`/`loader_calls` (SEM-020 enqueue) pero **no parsea atributos de markup HTML inline**. El `path_resolver` está sano — nunca lo invocan con esos hrefs porque el scanner no los extrae.
- **Generalización (clase, no caso ETCA):** afecta a cualquier lenguaje template que emita HTML inline — `.php`, `.twig`, `.blade.php`, `.erb`, `.jsp`, `.ejs`. La dimensión faltante en el análisis de dialectos D1-D11 fue **el tipo de archivo contenedor** (html-puro vs server-side-template), ortogonal a la forma del href.
- **Enfoque (a diseñar, NO hardcodear):** correr la extracción de markup (`<link>/<script>/<img>`) también sobre un conjunto de extensiones "html-bearing" además del dispatch por language — de forma **aditiva** al scanner PHP existente (no reemplazarlo: SEM-020 enqueue y require/include deben coexistir). Opciones a evaluar en diseño: (a) paso de extracción de markup que aplique por conjunto de extensiones; (b) el scanner del template compone el HtmlScanner sobre el texto. Punto de cambio: `compass/scanners/__init__.py:_build_scanner` + dispatch en `pipeline.py`. Solución por **clase de extensión**, jamás por nombre de archivo/carpeta.
- **No-regresión:** SEM-020 (enqueue WP en PHP) ya conecta 14 edges en ETCA — debe seguir idéntico. PHP-018/018b (require/include con `__DIR__`/variable) intactos. La paridad regex↔tree-sitter de HTML no se toca.
- **Testigo:** ETCA (6 nodos deben pasar a connected, sin tocar los 4 huérfanos reales: `_shared.css` legacy, `__shared.css` duplicado, `_mejoras.css` propuesta, `style.css` manifest WP).
- **Prioridad ALTA:** es el gap frontend más ancho del portfolio — todo sitio PHP/template con HTML inline. Mayor impacto que la colisión de mounts (WEB-039), que es más nicho.
...
## CSS-049 — REABIERTO: no confirmado funcional (S25)
...
## JSON-058 — actualización de testigo (S25)

- **Segundo testigo confirmado:** `public/manifest.json` en clases.etca queda suelto — lo referencia el metadata de Next (`manifest` en `app/layout.tsx` / `<link rel="manifest">`) por path literal a JSON, edge no trazado. Se suma al testigo canónico `mapper_config.json` (self-scan). Refuerza la prioridad ALTA; el patrón aplica multi-stack (Python self-scan + Next/TS).

## Cruce anti-duplicación S25 — lo que NO se ticketizó (ya cubierto)

Hallazgos de S25 que NO generaron ticket nuevo porque ya están cubiertos (registro de la verificación, para no re-abrir):

- **Express (D7), FastAPI/Flask literal (D3/D6), `static_folder` variable (D4):** todos dentro del scope vigente de **WEB-039** (PLAN L62 los lista). No son tickets aparte; son casos a resolver al implementar WEB-039.
- **`.mcp.json` no debe escanearse:** ya resuelto por **IGN-059** ✅ (S24) — está en `ignore_patterns` de `mapper_config.json`. Criterio reconfirmado por Beto en S25 ("es elemento del harness"). Sin acción nueva salvo que se detecte reaparición en algún testigo (verificar IGN-059 vs nodo-fantasma GHOST-063 en ese caso).
- **62 `.sql` + `database-old-backup.ts` (clases.etca) y huérfanos legítimos de ETCA:** son **huérfanos reales** (hallazgo correcto de Compass), su foco/clasificación es **AUDIT-060**, no un bug de cobertura.
- **Alias `@/` de Next/TS:** verificado funcional (clases.etca 34/35 tsx connected). NO es gap — queda como testigo de no-regresión.
- **Micro-gap orphan-patterns:** `database-old-backup.ts` no auto-clasifica orphan porque `DEFAULT_ORPHAN_PATTERNS.name_suffixes` usa `_backup` (underscore) y el archivo usa `-backup` (guion). Menor; sin ticket propio salvo reaparición.
...
## Evidencia de campo S25 (graph.html ETCA + WP-plugin) — derivada a tickets

Observaciones directas de Beto sobre `graph.html` de etca.com.ar (imagen adjunta, scan 2026-06-15 01:23, nodes=134/edges=1222) y sobre el WP-plugin. Registradas acá como evidencia disparadora y DERIVADAS a su ticket destino. El diagnóstico de prioridad #3 trabajará SOBRE esta evidencia, no la reemplaza.

- **`themes/etca-aula/functions.php` ambiguous y aislado del cluster principal** (imagen: cluster derecho separado, `functions.php` naranja con ~14 edges `enqueue` a CSS/JS del tema, sin tocar el sitio). → **DERIVA a TIER-062** (loader/página top-level que emite edges pero no es "usado por nadie" → debería ser entry_point, no ambiguous) **y posible interacción con RES-003** (theme-implicit debería promover `functions.php` a entry_point; verificar por qué no aplica en este scan). Evidencia para el diagnóstico: el cluster del tema WP está correctamente conectado internamente pero el `functions.php` no se promueve.

- **`.vscode/settings.json` aparece como nodo** (en etca.com.ar Y en aula.etca.com.ar). → **DERIVA a un ticket de la clase IGN-059** (config de tooling/editor externo que no debe ser nodo). `.vscode/` no está en `ignore` hoy. PENDIENTE verificar en diagnóstico: ¿es que `.vscode/` no está en `ignore_folders`/`ignore_patterns`, o es nodo-fantasma vía edge-target (**GHOST-063**)? El fix difiere según cuál sea. Si es lo primero → extender la lista de IGN-059 con `.vscode/`. Nota: `aula.etca.com.ar` es un proyecto irregular y solo sirve como testigo de este caso `.vscode/settings.json`.

- **Sueltos en etca.com.ar: `sidebar.php`, `scripts/gen-hash.php`, `themes/etca-aula/style.css`, `server.js`** → **DERIVAN a AUDIT-060** (clasificación huérfano-real vs gap). Notas por nodo para el diagnóstico: `gen-hash.php` ya clasificado huérfano-real (script CLI, censo previo); `server.js` es el Node http server = entry_point no detectado (revisar detección de entry para servers Node); `style.css` del tema WP = huérfano theme-implicit (RES-003, ¿se promueve?); `sidebar.php` es NUEVO — `.php` suelto, posible include no trazado o markup no detectado → verificar si cae en MARKUP-061 (¿lo incluye otro `.php` por `require`/`include`?) o es gap de cobertura.

- **WP-plugin `etca-dashboard-plugin`: CSS/JS de `assets/` sin conectar salvo `tutor-form.js` (conexión dudosa)** — path `C:\IA_Workspace\clientes\ETCA\app\WP-plugin-gestion-etca\etca-dashboard-plugin\assets`. → **DERIVA a auditoría como testigo de MARKUP-061 + SEM-020.** Evidencia para el diagnóstico: verificar (a) si los assets se cargan por `wp_enqueue` (SEM-020 debería trazarlos) o por markup PHP inline (MARKUP-061); (b) por qué `tutor-form.js` conectó y los demás no — ¿conexión correcta o falso positivo? Candidato a testigo de las extensiones MARKUP-061 sin verificar si el plugin usa markup en `.php`.

> **Pendiente de prioridad #3 (próximo movimiento, ANTES de WEB-039):** diagnóstico en grafo real de estos 4 grupos de evidencia, derivando cada hallazgo confirmado a su ticket (TIER-062, IGN-nuevo/GHOST-063, AUDIT-060, MARKUP-061/SEM-020). El diagnóstico NO re-descubre — confirma o refuta sobre la evidencia ya registrada acá.